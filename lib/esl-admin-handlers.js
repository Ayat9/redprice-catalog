...
import {
  ...
  updateCatalogBindingAndPrice,
  uploadCatalogImageBase64,
} from './esl-catalog-store.js'
...
export async function handleEslAdminRequest(pathname, method, bodyText, search = '') {
  ...
  try {
    ...
    if (pathname === '/api/v1/esl/admin/catalog' && method === 'GET') {
      const searchText = qs.get('q') || ''
      const data = await getCatalogRows({ q: searchText })
      return out(200, data)
    }

    if (pathname === '/api/v1/esl/admin/catalog/update' && method === 'POST') {
      const body = JSON.parse(bodyText || '{}')
      const data = await updateCatalogBindingAndPrice({
        id: body.id,
    ...
    if (pathname === '/api/v1/esl/admin/catalog/create' && method === 'POST') {
      const body = JSON.parse(bodyText || '{}')
      const data = await createCatalogProduct(body)
      return out(200, data)
    }

    if (pathname === '/api/v1/esl/admin/catalog/edit' && method === 'POST') {
      const body = JSON.parse(bodyText || '{}')
      const data = await updateCatalogProduct(body)
      return out(200, data)
    }

    if (pathname === '/api/v1/esl/admin/catalog/delete' && method === 'POST') {
      const body = JSON.parse(bodyText || '{}')
      const data = await deleteCatalogProduct(body.id)
      return out(200, data)
    }

    if (pathname === '/api/v1/esl/admin/catalog/upload-image' && method === 'POST') {
      const body = JSON.parse(bodyText || '{}')
      const data = await uploadCatalogImageBase64({ dataUrl: body.dataUrl, filename: body.filename })
      return out(200, data)
    }

    if (pathname === '/api/v1/esl/admin/catalog/sync-1c' && method === 'POST') {
      const body = JSON.parse(bodyText || '{}')
      const data = await syncCatalogFrom1C(Array.isArray(body?.items) ? body.items : [])
      return out(200, data)
    }
